...
const CollaboratorCheckout: React.FC = () => {
  const { items, getTotalPrice, getTotalItems, removeItem, updateItemQuantity, clearCart } = useCart();
  const { collaborator, monthlyOrderCount, monthlyCap, updateMonthlyOrderCount } = useCollaboratorAuth();
  ...
  // Check if the monthly order limit has been reached
  const orderLimitReached = monthlyOrderCount >= monthlyCap;
  ...
  const onSubmit = async (values: CheckoutFormValues) => {
    ...
    if (orderLimitReached) {
      toast.error(`Você atingiu o limite de ${monthlyCap} pedidos este mês`);
      return;
    }
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          customer_name: collaborator.name,
          customer_email: collaborator.email,
          customer_department: collaborator.sector || 'Não especificado',
      ...
      <div className="container mx-auto py-6">
        ...
        {orderLimitReached && (
          <Alert variant="destructive" className="mb-6">
            ...
            <AlertDescription>
              Você já realizou o máximo de {monthlyCap} pedidos permitidos para este mês.
            </AlertDescription>
          </Alert>
        )}
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <p className="text-sm font-medium mb-1">Matrícula:</p>
                      ...
                      <div>
                        ...
                        <p>{collaborator?.sector || 'Não especificado'}</p>
                      </div>
                    </div>
                    ...
                    <Button 
                      type="submit" 
                      className="w-full" 
                      disabled={isSubmitting || orderLimitReached}
                    >
                      {isSubmitting ? (
                        <>
...
export default CollaboratorCheckout;
